...
const completeLesson = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const enrollment = await enrollmentService.completeLesson(req.userId, lessonId);
...
const uncompleteLesson = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const enrollment = await enrollmentService.uncompleteLesson(req.userId, lessonId);
    res.status(200).json({ enrollment });
  } catch (error) {
    res.status(400).json({ error: (error as Error).message });
  }
