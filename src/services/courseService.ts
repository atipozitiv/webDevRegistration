...
export const courseService = {
  async createCourse(courseData: CreateCourseData) {
    ...
    await course.save();
    return await course.populate(["author", "tags"]);
  },
  ...
  async getFavoriteCourses(userId: string, options: PaginationOptions = { page: 1, limit: 10 }) {
    ...
    const user = await UserModel.findById(userId).populate({
      ...
      populate: [
        { path: "author", select: "name surname username" },
        { path: "tags" },
      ],
      options: {
        skip,
